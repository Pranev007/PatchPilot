...
**6 of 12 repositories migrated to a green test suite (50%)** on `claude-sonnet-5` at `--effort high`.
...
- Cost per success: **$0.59**
- Whole benchmark: **$3.55**
...
| `python-humanize/humanize` | 437 | hit spend cap | 2 | $0.485 | $0.42 | 31 |
...
Not one repository exhausted its *iteration* cap. All six failures hit the
**spend** cap, set at $0.30-$0.50 because the entire benchmark ran on a $5
budget. They ran out of money, not out of ideas.

How much that matters is measurable, so it was measured. Five capped repos
were re-run with nothing changed but the cap:
...
| `python-humanize/humanize` | capped at 1 iter | capped at 2 iters |
| `pytoolz/toolz` | capped at 1 iter | capped at 1 iter |
| `PyCQA/pycodestyle` | capped at 1 iter | capped at 1 iter |

A 40% larger budget converted **one of five**. So the failures are not all
one dollar away from success: `boltons` was genuinely close, while `toolz`
and `pycodestyle` burned an entire budget inside a single iteration both
times and never reached a second test run. More budget buys those repos more
exploration, not more progress.

`Delgan/loguru` (1,328 tests) and `python-attrs/attrs` (1,161) were never
given a realistic budget at all -- their baselines alone take ~400s and their
...
this task a *low* per-repo cap with more repositories buys more successful
migrations per dollar than a high cap with fewer.

One caveat on the cap itself: it is checked before each model call, so a
single expensive call can overshoot it. Two repos above ran to $0.46 and
$0.49 against a $0.42 cap. It bounds spend, it does not pin it.
